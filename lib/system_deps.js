...
function cmdFound(cmd) {
  return (!child_process.spawnSync(cmd, {shell:true}).stderr.length);
}
...
function systemDeps(basePath) {
  ...
  if (cmdFound('php --version')) {
    ...
    if (cmdFound('composer --version')) {
      ...
      var lines = child_process.execSync('composer show -p', basePath).
        toString().split('\n');
      ...
      try {
        var output = child_process.execSync('php ' + path.resolve(
